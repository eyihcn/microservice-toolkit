...
//		FuliMqMessageConsumerAroundHandler annotation = getAnnotation(joinPoint);
		...
		FuliMqMessage fuliMqMessage = (FuliMqMessage) args[0];
		final String messageId = fuliMqMessage.getId();
		// 业务执行结果

		Object proceed = joinPoint.proceed();
		if (!(proceed instanceof ConsumeConcurrentlyStatusEnum)) {
			throw new RuntimeException("请返回业务消费消息结果枚举类型");
		}
		ConsumeConcurrentlyStatusEnum consumeResult = (ConsumeConcurrentlyStatusEnum) proceed;
