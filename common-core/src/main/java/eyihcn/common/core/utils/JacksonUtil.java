...
public class JacksonUtil {
	...
	public static void dumnToPrettyJson(Object obj) {
		...
		if (null == obj) {
			log.debug("dumnToPrettyJson is empty");
			return;
		}
		if (obj instanceof String) {
			try {
				Object o = defaultMapper.readValue((String) obj, Object.class);
				log.debug(toPrettyJsonFormat(o));
			} catch (IOException e) {
				e.printStackTrace();
			}
			return;
		...
		try {
			log.debug(toPrettyJsonFormat(obj));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
